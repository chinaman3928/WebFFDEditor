...
//TODO there are cases where i write uint32 instead of int32 since sizeof(uint32)-trivial difference but 
// to be faithful. also check in write.js

/* 
let ab = new ArrayBuffer(100);
let dv = new DataView(ab);
dv.setUint32(0, 255, true);
dv.setFloat32(4, 3.14, true);
dv.setInt8(8, -128, true);

const b = new Blob([ab]);
const bURL = URL.createObjectURL(b);

const clickThis = document.createElement("a");
clickThis.innerHTML = "clickThis to get blob";
clickThis.setAttribute("target", "_blank");
clickThis.setAttribute("href", bURL);
clickThis.setAttribute("download", "clickThisGenerated");
...
function parseCharacter() {
  let ret = {
    ...
    hasOpenPortal: undefined,      //OG
    portalDepth: undefined,        //OG
    portalPosition: undefined,     //OG
  ...
  if (GAME == "UR") {
    ret.portals = new Map();
    //TODO NOTE lmao this member is uint32, but DEFAULT_REALM, the typedef key, and AddPortal() are all int32
    ret.lastRealm = 0; //NOTE DEFAULT_REALM = 0
    if (parseBool()) //hasOpenPortal
      ret.portals.set(0, { portalDepth: parseInt32(), portalPosition: parseUint8Array(12) });
  }
...
function parseHistory() {
  ...
  if (GAME == "OG")
    ...
    ret.lastPopulationTime = Math.floor(ret.lastPopulationTime);
...
function parse() {
  ...
  if (GAME == "UR") {
    ...
    //TODO TODO TODO hardcore check. what to do?
...
//TODO undefineds angela 

const wobj =
{
  ab: new ArrayBuffer(robj.i * 2),
  dv: new DataView(ab),
  i: 0,
  resize: function() {
    ab = new ArrayBuffer(ab.byteLength * 2);
    const olddv = dv;
    dv = DataView(ab);

    for (let j = 0; j < i; ++j)
      dv.setUint8(j, olddv.getUint8(j));
  }
};

function writeBool(b) {
  if (wobj.i + 1 > wobj.ab.byteLength)
    wobj.resize();
...
function writeInt32(n) {
  if (wobj.i + 4 > wobj.ab.byteLength)
    wobj.resize();
...
function writeInt16(n) {
  if (wobj.i + 2 > wobj.ab.byteLength)
    wobj.resize();
...
function writeString(s, bits) {
  if (wobj.i + (bits == 16 ? 2 : 4) + s.length > wobj.ab.byteLength)
    wobj.resize();
...
function writeCharacter(c) {
  ...
  writeString(c.templateName);
  writeString(c.originalTemplateName);
  ...
  for (let i = 0; i < 2; ++i) {
    ...
    for (let j = 0; j < c.effects[i].length; ++i)
      writeEffect(c.effects[i][j]);
  }

  writeUint32(c.subItems.length);
  for (let i = 0; i < c.subItems.length; ++i)
    writeItem(c.subItems[i]);
...
function write() {
  ...
  for (let i = 0; i < 12; ++i)
    writeString(hotkeySpellAssignment[i]);
  ...
  if (GAME == "UR") {
    ...
    //TODO hash does its job-if i edit the file and keep the old hash, then the next load will trigger cheat?! try HC
    //TODO TODO TODO unsure what to do
    if (robj.version >= 2) {
